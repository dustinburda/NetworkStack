...
//// @return - true if Interval i is contained entirely in some interval in interval_set
//static bool get_mergeable(std::set<Interval>& interval_set, const Interval& i, std::vector<IntervalIt>& mergeable) {
//    // TODO limit search space of loop
//    for(auto it = interval_set.begin(); it != interval_set.end(); it++) {
//        auto end_points = *it;
//
...
//static void merge_and_insert(std::set<Interval>& interval_set, const Interval& i) {
//    std::vector<IntervalIt> mergeable;
//
//    auto contained = get_mergeable(interval_set, i, mergeable);
//
//    if(i[0] == i[1]) //empty interval
//        return;
//
